...
namespace Honey {
	...
	void EditorLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<KeyPressedEvent>(HN_BIND_EVENT_FN(EditorLayer::OnKeyPressed));
		dispatcher.Dispatch<MouseButtonPressedEvent>(HN_BIND_EVENT_FN(EditorLayer::OnMouseButtonPressed));
		dispatcher.Dispatch<WindowDropEvent>(HN_BIND_EVENT_FN(EditorLayer::OnWindowDrop));
	}
	...
	{
		// TODO(Ryan): if a project is dropped in, probably open it
	...
	{
		std::string filepath = FileDialogs::OpenFile("Honey Scene (*.honey)\0*.honey\0");
		if (!filepath.empty())
			OpenScene(filepath);
	}
	...
	void EditorLayer::OpenScene(const std::filesystem::path& path)
	{
		...
		if (path.extension().string() != ".honey")
		{
			HN_WARN("Could not load {0} - not a scene file", path.filename().string());
			return;
		}
	...
	{
		std::string filepath = FileDialogs::SaveFile("Honey Scene (*.honey)\0*.honey\0");
		if (!filepath.empty())
		{
	...
	{
		HN_CORE_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate);
	...
	void EditorLayer::OnDuplicateEntity()
	{
		...
		Entity selectedEntity = m_SceneHierarchyPanel.GetSelectedEntity();
		HN_CORE_WARN("Trying to duplicate Entity: {} ({})", selectedEntity.GetName(), selectedEntity.GetUUID());
		if (selectedEntity)
		{
